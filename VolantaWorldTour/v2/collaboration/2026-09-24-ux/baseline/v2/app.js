(() => {
  ...
  function renderProse() {
    ...
    $('#prose').innerHTML = `
      ...
      <div class="actions">
        <button type="button" class="btn ${done ? 'done' : 'primary'}" id="flownbtn">${done ? '✓ Geflogen' : 'Als geflogen markieren'}</button>
  ...
  function renderRail() {
    $('#rail').innerHTML = CH.map(c => {
      const pips = c.legs.map(id => {
        ...
        return `<button type="button" class="${cls}" data-leg="${id}" title="${esc(id)} · ${esc(l.f)} → ${esc(l.t)}" aria-label="${esc(id)} ${esc(l.f)} nach ${esc(l.t)}"></button>`;
      }).join('');
      return `<div class="act" data-ch="${c.id}"><div class="lbl"><b>${c.id}</b>${esc(c.title)} <b class="cnt"></b></div><div class="pips">${pips}</div></div>`;
    }).join('');
  ...
  const Chart = (() => {
    ...
    if (!window.d3 || !window.topojson) {
      ...
      return { draw() {}, styleLegs() {}, recenter() {} };
    }
    const d3 = window.d3;
    const W = 960;
    const H = 540;
    ...
    function draw() {
      ...
      const lbl = (view === 'world' || view === 'globe') ? [l.f, l.t] : [...chApts];
      ...
      hud.text(view === 'globe' ? 'Ziehen zum Drehen' : view === 'leg' ? 'Ringe: halbe und volle Leg-Distanz' : '');
      host.classList.toggle('globe', view === 'globe');
    }

    svg.call(d3.drag()
      .filter(ev => state.view === 'globe' && !ev.button)
      .on('drag', ev => {
    ...
    return { draw, styleLegs, recenter() { globeCenter = null; } };
  })();
  ...
  function rowHtml(l) {
    ...
    return `<tr data-leg="${l.id}" class="${isH ? 'hx' : ''}${l.id === cur().id ? ' sel' : ''}">
      <td><input type="checkbox" data-done="${l.id}"${state.flown.has(l.id) ? ' checked' : ''} aria-label="${esc(l.id)} als geflogen markieren"></td>
      <td class="num lft">${esc(legLabel(l))}</td>
      <td class="mono">${esc(l.f)} → ${esc(l.t)}</td>
  ...
  function renderPlan() {
    ...
    for (const l of LEGS) {
      ...
      if (q) {
        const a = AP[l.f];
        const b = AP[l.t];
        const hay = `${l.id} ${l.f} ${l.t} ${a.n} ${b.n} ${cityOf(a)} ${cityOf(b)} ${CATN[a.cc]} ${CATN[b.cc]}`.toLowerCase();
    ...
    $('#plan tbody').innerHTML = rows.join('') || '<tr class="grp"><td colspan="8">Keine Legs für diesen Filter.</td></tr>';
    $('#planinfo').textContent = `${n} von ${LEGS.length} Legs · Flugzeit-Schätzung: ${D.meta.formula.a320}; H160: ${D.meta.formula.h160}.`;
  }
  ...
  function renderScn() {
    ...
    const rows = list => `<div class="scroll"><table><thead><tr><th>ICAO</th><th>Ort</th><th>Produkt</th><th>Entwickler</th><th>Bewertung</th><th>Begründung</th><th>Leg</th></tr></thead><tbody>${list.map(([a, it, lid]) =>
      `<tr data-go="${lid}"><td class="mono">${esc(a.i)}</td><td>${esc(cityOf(a))}</td><td>${link(it)}</td><td>${esc(it.dev)}</td><td class="mono">${esc(it.rating || '')}</td><td>${esc(it.why || '')}${it.alt ? ` Alternative: <a href="${esc(it.alt.url)}" target="_blank" rel="noopener">${esc(it.alt.name)} ↗</a>` : ''}${installationNotes(it)}</td><td class="mono">${lid}</td></tr>`).join('')}</tbody></table></div>`;

    const owned = D.owned.map(o => {
      const lid = LEGS.find(l => l.t === o.icao || l.f === o.icao);
      return `<tr${lid ? ` data-go="${lid.id}"` : ''}><td class="mono">${esc(o.icao)}</td><td>${esc(o.name)}</td><td>${esc(o.dev)}</td><td>${o.used ? (lid ? lid.id : '✓') : '<span class="long">nicht in der Tour</span>'}</td><td>${esc(o.note || '')}</td></tr>`;
    }).join('');
    ...
    $('#panel-scn').innerHTML = `<h2>Szenerien: wenig kaufen, gezielt installieren</h2>
      ...
      <div class="scroll"><table><thead><tr><th>ICAO</th><th>Ort</th><th>Begründung</th><th>Leg</th></tr></thead><tbody>${stdok.map(([a, it, lid]) =>
        `<tr data-go="${lid}"><td class="mono">${esc(a.i)}</td><td>${esc(cityOf(a))}</td><td>${esc(it.why)}</td><td class="mono">${lid}</td></tr>`).join('')}</tbody></table></div></details>
  ...
  function wire() {
    ...
    $('#prev').addEventListener('click', () => select(state.idx - 1));
    $('#next').addEventListener('click', () => select(state.idx + 1));
    $('#jump').addEventListener('change', ev => selectId(ev.target.value));
  ...
  function fillSelects() {
    ...
    $('#jump').innerHTML = CH.map(c => `<optgroup label="${c.id} · ${esc(c.title)}">${c.legs.map(id => {
      const l = LEGS[legIdx.get(id)];
      return `<option value="${id}">${esc(legLabel(l))} · ${esc(l.f)} → ${esc(l.t)}${l.k === 'h' ? ' (H160)' : ''}</option>`;
    }).join('')}</optgroup>`).join('');
